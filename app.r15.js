...
function renderResumen() {
  ...
  safeRender(renderPeriodoKPIs);
}

function renderPeriodoKPIs() {
  ...
  document.getElementById('kpiPeriodoTotal').textContent = total;
  document.getElementById('kpiPeriodoGanados').textContent = ganados;
  document.getElementById('kpiPeriodoPerdidos').textContent = perdidos;
  document.getElementById('kpiPeriodoCierre').textContent = pct(ganados, cerrados);
}
...
// Por si algún lead quedó con la redacción vieja del Estado (de antes de que
// se simplificara el desplegable del Sheet), lo traducimos acá para que nunca
// desaparezca de los grupos del dashboard, pase lo que pase en el Sheet.
...
async function loadData({ isFirstLoad }) {
  try {
    ...
    data.rows.forEach(r => { r['Estado'] = normalizeEstado(r['Estado']); });
...
function isArchived(r) {
  if ((r['Estado'] || 'Nuevo') !== 'Nuevo') return false;
  if (daysSince(r['date']) <= ARCHIVE_AFTER_DAYS) return false;
  // Si le agendaron un próximo contacto hoy o a futuro, alguien lo está
  // recuperando activamente — no debe seguir en Archivado.
  const proximo = r['Fecha próximo contacto'];
  if (proximo) {
    const d = parseDate(proximo);
    if (!isNaN(d) && !isOverdue(proximo)) return false;
  }
  return true;
}
...
function renderKPIs() {
  ...
  const cerrados = rows.filter(r => r['Estado'] === 'Cerrado - Ganado' || r['Estado'] === 'Cerrado - Perdido').length;
  const ganados = rows.filter(r => r['Estado'] === 'Cerrado - Ganado').length;

  const vencidos = rows.filter(r => isOverdue(r['Fecha próximo contacto']) && r['Estado'] !== 'Cerrado - Ganado' && r['Estado'] !== 'Cerrado - Perdido').length;
...
function renderTendenciaChart() {
  ...
  rows.forEach(r => {
    ...
    if (!minWeek || weekStart < minWeek) minWeek = weekStart;
    if (!maxWeek || weekStart > maxWeek) maxWeek = weekStart;
  });

  // Rellenamos TODAS las semanas entre la primera y la última con datos,
  // aunque tengan 0 leads — así el gráfico no "salta" fechas.
  const keys = [];
  if (minWeek && maxWeek) {
...
function renderCrmAccordion() {
  ...
  displayGroups.forEach(estado => {
    ...
    if (group.length === 0) {
      ...
      note.textContent = `Archivado: leads que llevan más de ${ARCHIVE_AFTER_DAYS} días marcados como "Nuevo" sin haber sido contactados. Para recuperar uno: cámbiale el Estado, o simplemente agéndale una fecha de "Próximo contacto" — cualquiera de las dos lo saca de acá solo.`;
...
function renderProximos() {
  ...
  const rows = state.rows
    ...
    .filter(r => r['Estado'] !== 'Cerrado - Ganado' && r['Estado'] !== 'Cerrado - Perdido')
